...
const BOOK = [
  ...
  { name: "Grace Han Muir Hs" },
  { name: "Maia IYA" },
  { name: "Christian Stiker IYA" },
  { name: "Caleb Newton" },
  { name: "Ava Newton" },
  { name: "Joshua Newton" },
  { name: "Selah Newton" },
  ...
  { name: "Daniel Kim" },
  { name: "Grace Kim" },
  { name: "Colin Inkyu Kim" },
  { name: "Sid Chowdhury A2F USC IYA" },
  { name: "Diego Delgado Muir Hs CC Prez" },
...
test("a given name that is also a label is not eaten mid-name", () => {
  const s = splitName("Christian Stiker IYA", corpus);
  assert.equal(s.name, "Christian Stiker");
  assert.deepEqual(s.tags, ["IYA"]);
});

test("a stack of labels all come off, and the name stays whole", () => {
  const s = splitName("Sid Chowdhury A2F USC IYA", corpus);
  assert.equal(s.name, "Sid Chowdhury");
...
test("the sticky tail needs no vocabulary for what follows a label", () => {
  ...
  const s = splitName("Diego Delgado Muir Hs CC Prez", corpus);
  assert.equal(s.name, "Diego Delgado");
...
test("a family surname is never reported as a label", () => {
  ...
  for (const n of ["Caleb Newton", "Ava Newton", "Joshua Newton"]) {
...
test("a three-part name keeps its surname", () => {
  ...
  const s = splitName("Colin Inkyu Kim", corpus);
