...
Tyler. Two of them are called Tyler Larsen and two are called Tyler Law, and one
of the Tyler Larsens was also stored twice because he edited his LinkedIn vanity
URL between two exports.

A message once went out to Tyler Larsen, who runs Trojan Tech Solutions with
Caleb, carrying things Tyler Law had said. Tyler Law is Colin's friend from a
...
```bash
people brief "Tyler Larsen"       # the gate: run this before you write a word
```
...
Searching "Tyler Larsen" returns ten of them: Scale AI in San Francisco, TJX in
...
```bash
people brief "Tyler Larsen"                   # fetches it automatically if missing
