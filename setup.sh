...
for _tool in list-audit list-gate kit-debt handoff-check learn durable-check corpus preflight gtme-graph gtme-math gtme-library gtme-learning clay-fixture-check review-gate task-graph graph-fuse work-ledger fanout site-fast untrusted-screen model-route intro list-sift ux-do decisions; do
  if [ -f "$SCRIPT_DIR/bin/$_tool" ]; then
    link_tool "$_tool"
