...
public final class OverlayWindow: NSPanel {
    ...
    ///
    /// Not `NSScreen.main`: main is the screen holding the key window, and
    /// this panel is never key, so on a second display it reported whichever
    /// screen the person had last clicked in. The glass would then cover the
    /// other monitor and every surface would be laid out against the wrong
    /// bounds.
    ...
    /// the one with the menu bar, always.
    ///
    /// It used to be the display the pointer was on. With a monitor stacked
    /// over the laptop screen, every drift of the mouse down into the laptop
    /// took the whole HUD with it, mid-answer. Asked for on 2026-09-20: "make
    /// it where its always locked on the persons main display for now". The
    /// first screen in `NSScreen.screens` is that display by contract.
