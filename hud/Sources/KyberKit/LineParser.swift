...
/// Kyber Lines, ported from `src/core/lines.ts`.
///
...
///     a <x> <y> [act=true]               move the agent's pointer; `a off`
///
...
public enum LineParser {
    ...
    public static func parse(_ line: String) throws -> Op? {
        ...
        case "a":
            ...
            //
            // Not `k`: the display already sends `k down` and `k up` for the
            // talk key, and one letter meaning two things by direction is a
            // trap for whoever reads a socket log.
            //
            // The agent's pointer. Same coordinates as `m`: screen points,
            // top-left origin, which is what AXPosition reports.
            if tokens.count == 2, tokens[1] == "off" { return .agentCursorOff }
            guard tokens.count >= 3,
                  let x = Double(tokens[1]), let y = Double(tokens[2])
            else {
                throw LineParseError.malformed(
                    "`a` needs two numbers, or `off`", line: trimmed)
            }
            let act = tokens.dropFirst(3).contains("act=true")
            return .agentCursor(point: CGPoint(x: x, y: y), act: act)
