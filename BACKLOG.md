...
| # | Item | Status | Notes |
| --- | --- | --- | --- |
| 106 | **Math across the whole kit, not just design** | open | Caleb, 2026-09-23, after the design math layer landed: *"It's applicable to everything. Understanding a person, understanding any concept, understanding any software."* The design work is the pilot and is real: `ux-engine/bin/ux-modes` decomposes a page into motion modes and separated a reference page (effective rank **2.39**, 7 distinct parallax rates) from a bad one (rank **1.01**, 253 of 253 moving pairs correlating >0.98). Start from [ux-engine/HANDOFF.md](../../ux-engine/HANDOFF.md) and the prompt at `~/Downloads/codex-math-survey-prompt-2026-09-23.md`, which already carries the four traps that each produced a confident wrong answer. **The standard, which is the whole item:** a technique earns its place only when it replaces a judgment currently being faked. Effective rank replaced "feels busy". Anything replacing nothing is decoration. Deliverable is `ux-engine/research/MATH-SURVEY.md`, ranked by expected value, **leading with the refusal list**, because saying no is most of the work |
| 107 | **The long list: everything math could be paired with** | open | Caleb's own framing, 2026-09-23: *"making an insanely long list of things for math to be paired up with."* Depends on #106 for the filter, otherwise it becomes a wish list. Each entry needs the same four fields as #106: what it computes, the faked judgment it replaces, the falsifier, and the cheapest experiment. **An entry with no falsifier is not an entry.** Candidate surfaces already in the kit: retrieval ranking, skill routing, outreach sequencing, the fitness loop in #0, the HUD's attention model, comedy scoring |
| 108 | **Neural mapping and computational neuroscience** | open | Caleb, 2026-09-23. Karthik on the same thread: *"therell be some deep diff eq and math work amongst all this with the neural mapping so would love for you to know that really well."* **Pairs with #106 and should not start before it**, or it becomes a reading list. Note Caleb's stated position in the same conversation: he is not in the loop at this level yet and wants to be when it needs judgement |
| 109 | **The open brain model, and predicting Caleb's reactions** | open | Caleb, 2026-09-23: *"Open source brain repo like the meta thing that j was released, maybe from that we can predict caleb's reaction to things bc of everything we gave him???"* **NEEDS IDENTIFYING FIRST: which Meta release he means is not recorded and was not guessed.** Ask him or find it before designing anything. **Hard requirement if it gets built:** a shuffled-label control. [[project_brain_retrieval]] records that a typed-edge graph over the second brain already died to exactly that control, and a reaction predictor trained on his own corpus is the single easiest thing in this kit to fool yourself with. See also `feedback_seal_the_test_set`: self-scored numbers are not measurements |
| 110 | **Make this an actual model, not a pile of files** | open | Caleb, 2026-09-23: *"we hv some rl concepts but it could be way more mathematical and systematic and intentional, a lot of our processes are kinda jank lol."* The honest version of the complaint: the kit has 105 skills, ~29 hooks and many tools with no stated formal relationship between them. Overlaps #0 (the learning loop that has never moved) and #3 (skill descriptions cannot route), and both are symptoms of the same missing structure. **Do #106 first** so the formalism is chosen rather than assumed |
| 111 | **Finish the vector-transition** | blocked | Caleb, 2026-09-23: *"Finish doing the vector-transition we had started! Especially applicable bc of the math we are implementing... how diff aspects of chewbacca relate to other aspects, deep math stuff here."* **BLOCKED: the term appears nowhere.** Grepped the whole Chewbacca repo and `~/second-brain` for `vector.transition` and `vector transition`, zero hits in either, so it was either named differently or lives in a transcript. Nearest candidates found: the steering-vector note at BACKLOG.md line 93, and `brain-retrieval`, where vectors took grep from 12% to 68% recall. **Ask Caleb what this was before starting** |
| 112 | **Resourceful, creative, coachable, tasteful** | open | Caleb, 2026-09-23, and he flags creative as the hard one: *"we've been cooking with some proprietary ideas like making it prompt engineer itself in diff directions that add up to creative outcomes... maybe applying some of the same concepts from design theory?"* **That port already has a worked precedent, which is the useful part:** `ux-gen` partitions a design space into six mutually-refusing stances and draws once per cell, rather than asking for "the best" version, because research/13 measured reward models preferring typical outputs at alpha-hat 0.57. Generating along deliberately incompatible axes and selecting blind is exactly the mechanism he is describing, already built for design. Also read `feedback_never_judge_my_own_output`: pairwise not pointwise, and the generator never selects |
| 113 | **Deployed agents that continuously improve the kit** | open | Caleb, 2026-09-23, asking the cost question himself: *"maybe that would cost too much?"* **Answer the cost before building.** The kit already has the arithmetic: the `graph-engineering` skill carries Amdahl with this repo's own measured numbers (p = 0.932 on the test suite, 14.6x ceiling, 7.7x at N=15) and the rule that below p = 0.7 it is not worth the complexity. Also `feedback_one_job_not_six`: six local jobs took the machine to load 50 and `load-guard` now blocks it. **The binding constraint is #0, not compute:** an always-on improver with no working selection step is an archive with no selection, which is a museum |
| 114 | **Local models for the low-level work** | open | Caleb, 2026-09-23. Natural pair with #113, since the cost objection mostly dissolves if the always-on layer is local. Precedent in the kit: `hud-voice` already runs on the Neural Engine, and Gavin's constraint stands, *subscriptions are not an option, so on-device only* (item #12). Scope it by which jobs need no judgement, which is the same line Caleb drew on 2026-09-23 about where he does and does not need to be in the loop |
